...
public class GeminiService {
    ...
    /**
     * Generates a conversational response using the Gemini API, incorporating student profile context.
     *
     * @param chatHistory The list of previous chat messages.
     * @param userMessage The new user message.
     * @param studentProfileContext A summary of the student's past assessment reports.
     * @return The AI's response text.
     */
    public String generateChatResponse(List<Map<String, String>> chatHistory, String userMessage, String studentProfileContext) {
        ...
        try {
            ...
            systemReply.put("parts", List.of(Map.of("text", "I understand. I'll be a compassionate friend and listener.")));
            ...
            log.info("Sending request to Gemini API... (key starts with: {})", apiKey.substring(0, Math.min(8, apiKey.length())));
     ...
     * Returns a Map with: sentiment (-1 to 1), emotion, distressLevel (0-10), keyThemes
     */
    ...
    public Map<String, Object> keywordAnalysisFallback(String message) {
        ...
        for (Map.Entry<String, Float> entry : distressKeywords.entrySet()) {
            if (lower.contains(entry.getKey())) {
                ...
                matchCount++;
                if (entry.getValue() >= 0.7f) emotion = "sad";
                else if (entry.getKey().contains("anxi") || entry.getKey().contains("scared") || entry.getKey().contains("panic") || entry.getKey().contains("worry")) emotion = "anxious";
                else if (entry.getKey().contains("angry") || entry.getKey().contains("frustrated")) emotion = "angry";
            }
        }

        // Multiple keyword matches increase the distress level
        if (matchCount >= 3) maxDistress = Math.min(1.0f, maxDistress + 0.15f);
        if (matchCount >= 5) maxDistress = Math.min(1.0f, maxDistress + 0.1f);

        if (lower.contains("happy") || lower.contains("grateful") || lower.contains("great") || lower.contains("wonderful")) {
            emotion = "happy";
            maxDistress = 0.1f;
        }
